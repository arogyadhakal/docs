...
```bash cURL
curl -X POST "https://api.pdfextractor.io/api/v1/ask" \
...
const response = await fetch('https://api.pdfextractor.io/api/v1/ask', {
  method: 'POST',
  headers: {
...
url = "https://api.pdfextractor.io/api/v1/ask"
headers = {
    "Authorization": "Bearer pk_live_your_api_key",
    "Idempotency-Key": "unique-request-id-123"
}
